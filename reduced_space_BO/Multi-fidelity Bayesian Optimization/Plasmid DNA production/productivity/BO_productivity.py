...
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.64_36512_0.0000_320_1.pkl")
...
SPACE = [
    ...
    {'name':'conversion_main', 'type':'continuous','domain':(0.90,        0.98)},
...
def superpro_sim_hi(X01: torch.Tensor) -> torch.Tensor:
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
    ...
    for i in range(X_real.shape[0]):
        p = X_real[i]
        temp4 = excel.Application.Run(
            'SuperExcelMatlab',
            43200, 108000,
            float(p[0]), float(p[1]), 0.95, 144000,
            float(p[2]), float(p[3]), float(p[4]),
            150, 0.578072, 0.03,
            int(round(p[5])), 0.003, 0.02,
            int(round(p[6])), 0.001333, 0.05
        )
    ...
    wb.Close(False); excel.Application.Quit()
...
# def _superpro_single(x01: torch.Tensor) -> torch.Tensor:
#     # clone of notebook SuperPro2 for a single point
#     p = unnormalize_x(x01, SPACE)
#     p = p.reshape(-1, 1).cpu().numpy()

#     excelObject = win32com.client.Dispatch("Excel.Application")
#     excelObject.Visible = False
#     wb = excelObject.Workbooks.Open(os.path.join(os.getcwd(), "pDNA.xlsm"))
#     excelObject.Application.Run("BeforeSuperExcelMatlab")

#     temp3 = np.shape(p)[1]

#     F = np.zeros((8, 1, temp3), dtype=np.float64)
#     for i in range(temp3):
#         temp4 = excelObject.Application.Run(
#             "SuperExcelMatlab",
#             43200, 108000,
#             p[0, i], p[1, i], 0.95, 144000,
#             p[2, i], p[3, i], p[4, i],
#             150, 0.578072, 0.03,
#             np.round(p[5, i]), 0.003, 0.02,
#             np.round(p[6, i]), 0.001333, 0.05,
#         )
#         F[0, 0, i] = temp4[0]
#         F[1, 0, i] = temp4[1]
#         F[2, 0, i] = temp4[2]
#         F[3, 0, i] = temp4[3] / temp4[4]
#         F[4, 0, i] = temp4[4]
#         F[5, 0, i] = temp4[5]
#         F[6, 0, i] = temp4[6]
#         F[7, 0, i] = temp4[3] / (temp4[4] * (temp4[1] / 0.001))

#     excelObject.Application.Run("AfterSuperExcelMatlab")
#     wb.Close(False)
#     excelObject.Application.Quit()

#     return torch.from_numpy(F[1, 0, :]).unsqueeze(-1).to(dtype=dtype)


# def superpro_sim_hi(X01: torch.Tensor) -> torch.Tensor:
#     # allow batch input by calling the original logic point-by-point
#     if X01.dim() == 1:
#         return _superpro_single(X01)
#     results = [ _superpro_single(x) for x in X01 ]
