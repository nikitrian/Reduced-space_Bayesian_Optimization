...
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.24_8585_0.0002_512_1.pkl")
    ...
    try:
        ...
        for i in range(X_real.shape[0]):
            ...
            temp4 = excel.Application.Run(
                'SuperExcelMatlab',
                float(p[0]), float(p[1]), 0.0033, 0.02045, 0.95,
                float(p[2]), float(p[3]), 0.204507, 0.98, 150,
                float(p[4]), 0.03, np.round(p[5]), float(p[6]), 0.02,
                np.round(40), 0.001333, 0.05
            )
