...
HYSYS_FILE = os.environ.get("CARBON_HYSYS_FILE", HYSYS_FILE_DEFAULT)
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.21_5459_0.0100_639_1.pkl")
...
# Output root
out_root = os.path.join(BASE_DIR, "results_carbon")
...
# ----------------------
# Derived constants for the carbon case
# ----------------------
...
SPACE = [
    ...
    {"name": "recycle_ratio", "type": "continuous", "domain": (0.94, 0.995)},
...
# ----------------------
# HI: Aspen HYSYS carbon efficiency
# ----------------------
...
def _ensure_hysys():
    ...
    if not HYSYS_FILE:
        raise RuntimeError('HYSYS file path not set. Provide CARBON_HYSYS_FILE environment variable.')
...
def _evaluate_hysys_point(x_real: np.ndarray) -> float:
    ...
    inputs = list(map(float, x_real.tolist())) + list(CARBON_FIXED_INPUTS)
    try:
        sim.Visible = False
...
def carbon_sim_hi(X01: torch.Tensor) -> torch.Tensor:
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
# ----------------------
@torch.no_grad()
def carbon_ann_lo(X01: torch.Tensor) -> torch.Tensor:
    # X01: (n, D) in [0,1]^D
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
f_hi, f_lo = carbon_sim_hi, carbon_ann_lo
...
def sanity_check_once(x01: torch.Tensor):
    with torch.no_grad():
        y_lo = float(carbon_ann_lo(x01.unsqueeze(0)).item())
        y_hi = float(carbon_sim_hi(x01.unsqueeze(0)).item())
