...
HYSYS_FILE = os.environ.get("OPEX_HYSYS_FILE", HYSYS_FILE_DEFAULT)
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.17_5766_0.0013_512_1.pkl")
...
# Output root
out_root = os.path.join(BASE_DIR, "results_opex")
...
# ----------------------
# Derived constants for the OPEX case
# ----------------------
...
OPEX_INPUT_TEMPLATE = [
    ...
    7800.0,           # Pmeoh (fixed)
    None,             # Tmeoh
    0.0,              # reactor_mode (fixed)
    None,             # recycle_ratio
    float(MEOH_NOMINAL_VOL),  # Vmeoh (fixed)
    275.0,            # Tdme (fixed)
    1500.0,           # Pdme (fixed)
    0.5,              # xmeoh (fixed)
    0.5,              # xdme (fixed)
...
def _ensure_hysys():
    ...
    if not HYSYS_FILE:
        raise RuntimeError('HYSYS file path not set. Provide OPEX_HYSYS_FILE environment variable.')
...
def _evaluate_hysys_point(x_real: np.ndarray) -> float:
    ...
    for entry in OPEX_INPUT_TEMPLATE:
        if entry is None:
            try:
    ...
    if outputs_arr.size < 4:
        return HYSYS_FAILURE_VALUE

    value = outputs_arr.flat[3]
    if value is None:
        return HYSYS_FAILURE_VALUE
    try:
        value = float(value)
...
def opex_sim_hi(X01: torch.Tensor) -> torch.Tensor:
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
# ----------------------
@torch.no_grad()
def opex_ann_lo(X01: torch.Tensor) -> torch.Tensor:
    # X01: (n, D) in [0,1]^D
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
    ...
    # normalize inputs for the ANN using the +/-10% bounds it was trained on
...
f_hi, f_lo = opex_sim_hi, opex_ann_lo
...
def _dedup_mask(new_X, existing_X, atol=1e-6):
    ...
    # pairwise |x_i - y_j|_inf (max abs per row)
...
def run_mf_enum(bounds_x, f_hi, f_lo, f_star=None, tag="superpro"):
    ...
    for seed in range(num_seeds):
        ...
        while total_cost < budget and it < max_iters:
            ...
            if tagF=="HI":
                best_hi_so_far = min(best_hi_so_far, float(y_next.item()))
                steps_since_hi = 0
            else:
                steps_since_hi += 1
            ...
            if it % PROMOTE_EVERY_ITERS == 0:
                ...
                if n_prom > 0:
                    print(f"[promo] Evaluated {n_prom} LO points at HI (budget now {total_cost:.1f}).")
...
def sanity_check_once(x01: torch.Tensor):
    with torch.no_grad():
        y_lo = float(opex_ann_lo(x01.unsqueeze(0)).item())
        y_hi = float(opex_sim_hi(x01.unsqueeze(0)).item())
