...
HYSYS_FILE = os.environ.get("CAPEX_HYSYS_FILE", HYSYS_FILE_DEFAULT)
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.37_22670_0.0001_512_1.pkl")
...
# Output root
out_root = os.path.join(BASE_DIR, "results_capex")
...
# ----------------------
# Derived constants for the CAPEX case
# ----------------------
...
# ----------------------
# HI: Aspen HYSYS CAPEX
# ----------------------

CAPEX_INPUT_TEMPLATE = [
    None,             # h2_ratio
    None,             # Pmeoh
    None,             # Tmeoh
    None,             # reactor_mode
...
def _ensure_hysys():
    ...
    if not HYSYS_FILE:
        raise RuntimeError('HYSYS file path not set. Provide CAPEX_HYSYS_FILE environment variable.')
...
def _evaluate_hysys_point(x_real: np.ndarray) -> float:
    ...
    for entry in CAPEX_INPUT_TEMPLATE:
        if entry is None:
            try:
    ...
    if outputs_arr.size < 3:
        return HYSYS_FAILURE_VALUE

    value = outputs_arr.flat[2]
    if value is None:
        return HYSYS_FAILURE_VALUE
    try:
        value = float(value)
...
def capex_sim_hi(X01: torch.Tensor) -> torch.Tensor:
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
# ----------------------
@torch.no_grad()
def capex_ann_lo(X01: torch.Tensor) -> torch.Tensor:
    # X01: (n, D) in [0,1]^D
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
f_hi, f_lo = capex_sim_hi, capex_ann_lo
...
def sanity_check_once(x01: torch.Tensor):
    with torch.no_grad():
        y_lo = float(capex_ann_lo(x01.unsqueeze(0)).item())
        y_hi = float(capex_sim_hi(x01.unsqueeze(0)).item())
