...
HYSYS_FILE = os.environ.get("ENERGY_HYSYS_FILE", HYSYS_FILE_DEFAULT)
ANN_PKL_PATH     = os.path.join(BASE_DIR, "ann_ACC_0.31_13404_0.0005_265_1.pkl")
...
# --- Periodic LO-to-HI promotion knobs ---
...
# Output root
out_root = os.path.join(BASE_DIR, "results_energy")
...
# ----------------------
# Derived constants for the energy case
# ----------------------
...
SPACE = [
    ...
    {"name": "Pdme",          "type": "continuous", "domain": (1000.0, 2000.0)},
    {"name": "xmeoh",         "type": "continuous", "domain": (0.0, 1.0)},
    {"name": "xdme",          "type": "continuous", "domain": (0.0, 1.0)},
    {"name": "n_trays1",      "type": "continuous", "domain": (57.0 * 0.8, 57.0 * 1.2)},
    {"name": "feed_loc1",     "type": "continuous", "domain": ((44.0 / 57.0) * 0.8, (44.0 / 57.0) * 1.2)},
]
D = len(SPACE)
...
# ----------------------
# HI: Aspen HYSYS energy efficiency
# ----------------------

# Order-sensitive template (None entries are replaced by decision variables)
ENERGY_INPUT_TEMPLATE = [
    None,             # h2_ratio
    None,             # Pmeoh
    None,             # Tmeoh
    None,             # reactor_mode
...
def _ensure_hysys():
    ...
    if not HYSYS_FILE:
        raise RuntimeError('HYSYS file path not set. Provide ENERGY_HYSYS_FILE environment variable.')
...
def _evaluate_hysys_point(x_real: np.ndarray) -> float:
    ...
    for entry in ENERGY_INPUT_TEMPLATE:
        if entry is None:
            try:
                inputs.append(next(values_iter))
            except StopIteration:
                raise RuntimeError("Insufficient decision variables when building HYSYS inputs.") from None
        else:
            inputs.append(float(entry))
    try:
        next(values_iter)
        raise RuntimeError("Received extra decision variables when building HYSYS inputs.")
    except StopIteration:
        pass
    try:
        sim.Visible = False
    ...
    if outputs_arr.size < 2:
        return HYSYS_FAILURE_VALUE

    value = outputs_arr.flat[5]
    if value is None:
        return HYSYS_FAILURE_VALUE
    try:
        value = float(value)
...
def energy_sim_hi(X01: torch.Tensor) -> torch.Tensor:
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
# ----------------------
@torch.no_grad()
def energy_ann_lo(X01: torch.Tensor) -> torch.Tensor:
    # X01: (n, D) in [0,1]^D
    if X01.dim() == 1:
        X01 = X01.unsqueeze(0)
...
f_hi, f_lo = energy_sim_hi, energy_ann_lo
...
def sanity_check_once(x01: torch.Tensor):
    with torch.no_grad():
        y_lo = float(energy_ann_lo(x01.unsqueeze(0)).item())
        y_hi = float(energy_sim_hi(x01.unsqueeze(0)).item())
